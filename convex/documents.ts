...
export const archive = mutation({
    ...
    handler: async (ctx, args) => {
        ...
        if(existingDocument.userId !== userId){
            throw new Error("Доступ не разрешён");
        }
...
export const getTrash = query({
    handler: async(ctx) => {
        ...
        const archiveDocuments = await ctx.db.query("documents").withIndex("by_user", (q) => q.eq("userId", userId)).filter((q) => q.eq(q.field("isArchived"), true)).order("desc").collect();
